    ...
    parseGoogleDocJSON(res) {
      ...
      for (var i = 0; i < res.feed.entry.length; i++) {  
        ...
        if (playerlevel.length > 0 ) {
          ...
          progression['unlocks'] = {};
          for ( let unlockIndex = 1; unlockIndex <= 5; unlockIndex ++) {
           
            let propnameUnlockSubcategory = 'gsx$unlock' + unlockIndex + 'subcategory';
            let propvalueUnlockSubcategory = res.feed.entry[i][propnameUnlockSubcategory].$t;
      
            if ( propnameUnlockSubcategory.length > 0 && (propvalueUnlockSubcategory !== 'Null')) {
              
                let propnameUnlockAmount = 'gsx$unlock' + unlockIndex + 'amount';
                let propnameUnlockXP = 'gsx$unlock' + unlockIndex + 'xp';
   
                let unlock = {};
                

                unlock['amount'] = parseInt( res.feed.entry[i][propnameUnlockAmount].$t, 10);
                unlock['xp'] = parseInt( res.feed.entry[i][propnameUnlockXP].$t, 10);
                
                progression['unlocks'][ propvalueUnlockSubcategory ] = unlock;
            }
          }
